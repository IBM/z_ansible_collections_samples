# Ansible Playbooks for Appliance Installation in Standalone Mode with MFA
...
- Export the ACC-admin username and default password. Additionally, set and
  export a new password for the ACC admin and a default password for the appliance-owner. Keep in mind, the ACC API will be used to set these new passwords for
...
- Modify the variables in the file `admin_vars.yaml`.
  - **Note**: Be sure the `acc_url` in the `admin_vars.yaml` file points to the IP of the ACC LPAR. Keep the port to 8081.
...
As an ACC-admin, after updating the appropriate variables in the `admin_vars.yaml`file. Run the appropriate playbook depending on the number of appliance LPARs to be installed:

- **For one LPAR**, run the playbook:

    ```bash
    ansible-playbook 02a_assign_1_lpar.yaml
    ```

- **For two LPARs**, run the playbook:

    ```bash
    ansible-playbook 02b_assign_2_lpar.yaml
    ```

This action will just assign LPARs to the appliance-owner. The LPARs will not be
...
These playbooks will prompt the user to select the network card type for each SSA LPAR:
...
Ensure the appropriate `vlan*_id` variables are set in the `admin_vars` file if VLAN tagging is required.
...
These playbooks will also prompt the user to select the appropriate disk type based on the current system configuration:

- For FCP disk, ensure that valid values for 'wwpn*' and 'lun*' are provided in the `admin_vars` file.
...
- Modify the variables in the file `owner_vars.yaml`.
  - **Note**: Be sure the `acc_url` in the `owner_vars.yaml` file points to the IP of the ACC LPAR. Keep the port to 8081.
...
assigned to the appliance-owner are either in SSC Installer mode or in
SSC mode. If the LPARs are deactivated, contact the ACC-admin or the HMC-admin
to bring these LPARs into the right state.
